...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="/ubahpengelola/{{$datapengelola->id}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form form-group">
                </div>
                <small>Nama Lengkap</small> <br>
                <input type="text" name="name" id="" class="form form-control" value="{{$datapengelola->name}}"> <br>
                @error('name')
                    <small>{{$message}}</small><br>
                @enderror
                <small>Email</small> <br>
                <input type="text" name="email" id="" class="form form-control" value="{{$datapengelola->email}}"> <br>
                @error('email')
                    <small>{{$message}}</small><br>
                @enderror
